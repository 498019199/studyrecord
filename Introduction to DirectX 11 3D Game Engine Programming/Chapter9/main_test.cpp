...
{
public:
    RenderablePlaneTex(float width, float depth, float texU, float texV, const std::string& DDS, const std::string& mode)
    {
...
void CreateScene()
{
    ...
    // 初始化地板
    auto floor = new RenderablePlaneTex(20.0f, 20.0f, 5.0f, 5.0f, "floor.dds", "Basic_2D");
    ...
    for (int i = 0; i < 4; ++i)
    {
        auto wal = new RenderablePlaneTex(20.0f, 8.0f, 5.0f, 1.5f, "brick.dds", "Basic_2D");
    ...
    // 初始化地板
    auto mirror = new RenderablePlaneTex(8.0f, 8.0f, 1.0f, 1.0f, "ice.dds", "Basic_2D");
    Context::Instance().WorldInstance().AddRenderable(mirror);
    auto rotM = MathWorker::ToMatrix(rotator(-MathWorker::PIdiv2, 0.f, 0.f));
    movement = MathWorker::Translation(0.0f, 0.0f, 10.0f);
    mirror->TransformToWorld(movement * rotM* mirror->TransformToParent());
    mirror->Update(0.f);
}
